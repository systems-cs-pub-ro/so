...
so_exec_t *so_parse_exec(char *path)
{
	...
	if (readHeader(hFile, 0, (char *)&dosHdr, sizeof(IMAGE_DOS_HEADER)) < 0) {
	...
	if (readHeader(hFile, dosHdr.e_lfanew, (char *)&peHdr, sizeof(IMAGE_NT_HEADERS)) < 0) {
		fprintf(stderr, "Couldn't read PE DOS header\n");
		goto out;
	}
	...
	if (readHeader(hFile, dosHdr.e_lfanew + sizeof(IMAGE_NT_HEADERS),
			(char *)secHdr, nr_sections * sizeof(IMAGE_SECTION_HEADER)) < 0) {
		fprintf(stderr, "Couldn't read PE DOS header\n");
		goto out_free;
	}
	...
	exec->entry = peHdr.OptionalHeader.ImageBase + peHdr.OptionalHeader.AddressOfEntryPoint;
	exec->segments_no = peHdr.FileHeader.NumberOfSections + 1 /* an extra segment for headers */;
	...
	for (i = 0; i < peHdr.FileHeader.NumberOfSections; i++) {
		...
		seg->vaddr = peHdr.OptionalHeader.ImageBase + sec->VirtualAddress;
...
void so_start_exec(so_exec_t *exec, char *argv[])
{
	IMAGE_NT_HEADERS *peHdr = (IMAGE_NT_HEADERS *)
		(((IMAGE_DOS_HEADER *)(exec->base_addr))->e_lfanew + (DWORD)(exec->base_addr));
...
static BOOL SolveImports(uintptr_t loadBase, IMAGE_NT_HEADERS *peHdr)
{
	IMAGE_DATA_DIRECTORY *dir = &peHdr->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT];
	DWORD *oft;
	DWORD *ft;
	...
	while (impDesc->OriginalFirstThunk) {
		...
		for (; *oft; oft++, ft++) {
			if (*oft & 0x80000000) {
				impOrd = *oft;
				impOrd &= 0xffff;
				funcAddr = (DWORD)GetProcAddress(hModule, (LPCSTR)impOrd);
				if (!funcAddr) {
					fprintf(stderr, "Couldn't find the function %d: %d\n", impOrd, GetLastError());
					return FALSE;
				}
				*ft = funcAddr;
			}
			else {
				impName = (IMAGE_IMPORT_BY_NAME *)((DWORD)loadBase + *oft);
				funcAddr = (DWORD)GetProcAddress(hModule, impName->Name);
				if (!funcAddr) {
					fprintf(stderr, "Couldn't find the function %s: %d\n", impName->Name, GetLastError());
					return FALSE;
				}
				*ft = funcAddr;
			}
